...
class MCChatVC: JSQMessagesViewController, OneMessageDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        if let recipient = recipient {
            ...
            self.navigationItem.setRightBarButtonItem(UIBarButtonItem(barButtonSystemItem: .Add, target: self, action: #selector(MCChatVC.addRecipient)), animated: true)
            if firstTime {
                firstTime = false
                addRecipient()
            }
        ...
        if textView.text.characters.count == 0 {
            ...
            if !isComposing {
                ...
                OneMessage.sendIsComposingMessage((recipient?.jidStr)!, completionHandler: { (stream, message) -> Void in
                    self.timer = NSTimer.scheduledTimerWithTimeInterval(4.0, target: self, selector: #selector(MCChatVC.hideTypingIndicator), userInfo: nil, repeats: false)
                })
            } else {
                self.timer = NSTimer.scheduledTimerWithTimeInterval(4.0, target: self, selector: #selector(MCChatVC.hideTypingIndicator), userInfo: nil, repeats: false)
            }
